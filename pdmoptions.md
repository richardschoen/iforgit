...
From the version list screen, the user can ```Opt 2 - Edit``` the version, ```Opt 5 - Browse``` the version or ```Opt 6 - Restore``` the version to library: ```IFORGITTMP/USERID```. Once a member is restored, the developer can ```Opt 7 - Work with Temporary Restored Members``` in ```IFORGITTMP/USERID``` to work with restored members using PDM.   
