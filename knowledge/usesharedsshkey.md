...
This article discusses setting up your local Git repository config (.git/config) for each Git repo to 
utilize a private key stored in the user's home directory. Or a private key file can be placed 
...
Next, edit the ```.git/config``` file for each relevent GitHub repository to point to the user's 
...
The entire ```.git/config``` may look as follows:
```
[core]                                                   
